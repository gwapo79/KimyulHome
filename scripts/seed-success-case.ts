...
async function main() {
    ...
    const successCase = await prisma.successCase.create({
        data: {
            ...
            lawyer: "김법무 변호사",
            result: "전액 회수 성공",
            background: "의뢰인은 서울시 강남구 소재 아파트에 전세금 4억원으로 거주하던 중, 임대인이 계약 만료 후 보증금 반환을 거부하며 연락을 두절하는 상황에 직면했습니다. 임대인의 재산 상태가 불투명한 상황에서 신속한 법적 대응이 필요한 상황이었습니다.",
    ...
    console.log('Use this URL to view:', `http://localhost:3000/legal/success-cases/${successCase.id}`);
}
