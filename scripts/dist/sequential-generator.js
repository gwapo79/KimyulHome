...
async function generateImage(prompt, filename) {
    ...
    try {
        ...
        const filePath = path_1.default.join(process.cwd(), 'public/images/success-cases', filename);
...
async function main() {
    ...
    // Read cases
    const data = JSON.parse(fs_1.default.readFileSync('success_cases.json', 'utf-8'));
    let processed = 0;
    for (const item of data) {
        ...
        const pick = (arr) => arr[item.id.charCodeAt(0) % arr.length];
