...
import { Search, Plus, Gavel, MoreVertical, Edit } from 'lucide-react';
import { useState } from 'react';
import Link from 'next/link';
import { SuccessCase } from '@prisma/client';

interface SuccessListClientProps {
    initialCases: SuccessCase[];
}

export default function SuccessListClient({ initialCases }: SuccessListClientProps) {
    const [searchQuery, setSearchQuery] = useState('');

    const filteredCases = initialCases.filter(c =>
        c.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (c.lawyer && c.lawyer.toLowerCase().includes(searchQuery.toLowerCase()))
    );

    const getResultBadge = (result: string | null) => {
        if (!result) return null;
        if (result.includes('승소') || result.includes('무혐의') || result.includes('인가')) {
        ...
        if (result.includes('화해') || result.includes('조정')) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">로펌 승소 실적 및 판결문 등록 (DB 연동됨)</p>
                </div>
                ...
                <table className="table-fixed w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-slate-50 border-b border-slate-200">
                            <th className="w-[40%] px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider pl-8">사건명 (Title)</th>
                            <th className="w-[15%] px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">분야</th>
                            <th className="w-[15%] px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">결과</th>
                            <th className="w-[15%] px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">담당변호사</th>
                            <th className="w-[15%] px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">관리</th>
                        </tr>
                    </thead>
                            ...
                            <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4 pl-8">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <Link href={`/admin/success/${item.id}`} className="hover:underline hover:text-blue-600 block min-w-0">
