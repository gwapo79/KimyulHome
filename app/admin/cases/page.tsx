...
import { MOCK_CASES, CaseStatus } from '@/data/mock_cases';
import { Search, Plus, Filter, Briefcase, Calendar, ChevronRight } from 'lucide-react';
...
export default function CasesPage() {
    ...
    const getStatusBadge = (status: CaseStatus, label: string) => {
        const colors = {
        ...
        return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${colors[status] || 'bg-gray-100'}`}>{label}</span>;
    };
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">진행 중인 사건 현황 및 타임라인 관리</p>
                </div>
